...
- ✅ 1. HUD 每日订单进度：金币行下方常驻「订单 鲢鳙 0/4」可点开订单页（4e6c8fd）
- ✅ 2. 鱼篓排序/筛选：最新/价值/品阶/重量 + 订单鱼筛选（d263e15）
- ✅ 3. 图鉴长期徽章：每种鱼 集齐10条/巨物/完美 徽章 + 进度，存档 v7（dex 4 元组，旧档迁移）
- ✅ 4. 多类型订单：species/tier/weight/perfect 四类（perfect 需鱼饵≥2），匹配/标题/HUD短签/奖励统一，存档保留 kind/tier/minw，旧档默认 species
- ✅ 5. 数值平衡：鱼竿成本 40×1.8^n → 200×2.0^n，回本随级递增（balance_probe.gd 探针）
- ✅ 文档：README/白皮书/art_requests.md 全面同步
- ✅ 6. 专注模式：设置开关，停小动物事件+抑制飘字+场景轻微变暗，存档持久化


## 数值再平衡复查结论（2026-06-14 晨，含鱼钩）

balance_probe 复查（已纳入鱼钩双钩）：经济健康，**本轮无需调整**。
- 鱼竿回本 0.3→7.3 分递增（核心线，早期便宜后期成墙）
- 鱼饵回本 4.3 / 8.6 / 17.4 分（中期线）
- 鱼钩回本 4.9 / 14.8 / 46.3 分（后期奢侈线，顶钩 60000/约46分回本＝合理终局目标）
- 全裸(rod1/蚯蚓/基础钩)≈746 金/分 → 全满(rod10/秘制/双叉)≈14807 金/分，×19.9 成长曲线
- 离线 8h 受背包格数截断（健康，促回访卖鱼+扩容）
- 三线分工定价错落（竿→饵→钩，由便宜到奢侈），无重叠失衡
