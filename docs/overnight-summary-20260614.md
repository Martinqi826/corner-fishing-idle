...
| 14 | **首次引导** 新玩家首启「欢迎」卡讲清核心循环，看过即存档标记不再弹 | 上手体验打磨 | 见 git |
| 15 | **bug巡检** 修复多类型订单的「订单鱼」筛选(按_order_matches) + 双钩满篓溢出边界测试 | 打磨/健壮性 | 见 git |
...
| 20 | **回归巡检** 全量回归 22 段 0 失败 + 截图无溢出 + 面板拖动钳制复查健全；修截图工具引导遮挡 | 无回归确认 | 见 git |
...
- `godot --headless -s tools/validate_game.gd` → **0 失败**（22 段：数据/分布/体重价格/背包卖鱼/排序筛选卖杂鱼/鱼竿/鱼饵星级/鱼钩双钩/成就(23项)/鱼贩/鱼汛/周目标/今日统计/每日订单(多类型)/存档v2往返/原子写损坏回退/v1迁移/离线/动态层/专注模式）。
