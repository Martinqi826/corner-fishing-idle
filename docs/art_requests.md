...
鱼类图标必须遵守 `docs/fish_icon_art_standard.md`。旧有 `carp`、`koi`、
`bass`、`mandarin` 是唯一生产级参考，不是宽泛风格参考。任何新增鱼图标
如果无法与这几张放在同一行而不显得质感更差，就不能进入
`assets/art/fish/<id>.png` 正式运行路径，只能留在 WIP 或继续使用程序回退图标。
